...
def errore():
	lento(RR+"¡Ocurrió un error inesperado!")
...
# Preparación para identificación de dato existente
	...
	while fila < 1000:
		...
		while db1 != None:
			...
			cnt4 = (f"W{tom}")
			db4 = hoja[cnt4].value
			print (BB+"Código:"+WW, db1 +BB, "| Nombres:"+WW, db2, db3 +BB, "| Razon DB:"+WW, db4)
			break
		
		else:
			print (GG+"Esos son todos los datos existentes\n")
			break
	input(BB+"Presiona cualquier boton para regresar al Menú: "+ WW)
	GUI()
...
# Subalternativa 2 - limpiar celdas
			...
			medio(RR+"Escogiste una alternativa incorrecta!, vuelve a intentarlo\n")
			opt4()
...
def GUI():
	os.system("clear")	
	arch01 = open("db.txt","r")
	arch02 = arch01.read()
	print ("Nombre de base de datos:", arch02)
	tlt ="""██████╗ ██████╗           ██╗   ██╗████████╗ █████╗ \n██╔══██╗██╔══██╗          ██║   ██║╚══██╔══╝██╔══██╗\n██████╦╝██║  ██║  █████╗  ╚██╗ ██╔╝   ██║   ███████║\n██╔══██╗██║  ██║  ╚════╝   ╚████╔╝    ██║   ██╔══██║\n██████╦╝██████╔╝            ╚██╔╝     ██║   ██║  ██║\n╚═════╝ ╚═════╝              ╚═╝      ╚═╝   ╚═╝  ╚═╝   v2.81"""
	print (YY+tlt)
...
# Error de valor en variable (ValueError)		
	except ValueError:
		print (RR+"¡Ocurrió un error inesperado!\n"+WW+"Vuelve a intentarlo")
		os.system("sleep 2")
		GUI()

# Interrupción de entrada (KeyboardInterrupt)
	except KeyboardInterrupt:
		lento("\nVolviendo al Menú principal...")
		os.system("sleep 1")
		GUI()

# Error de caracter (AttributeError)
	except AttributeError:
		lento("\nOcurrió un error, Volviendo al Menú principal...")
		os.system("sleep 1")
		GUI()

# Error de Indexacion (IndexError)
	except IndexError:
		lento("\nOcurrió un error, Volviendo al Menú principal...")
		os.system("sleep 1")
		GUI()

# Error de STR caracter (TypeError)
	except TypeError:
		sutil("ERROR | Procura ingresar un numero y no una letra")
		GUI()

# Error de programación (NameError)
	except NameError:
		sutil(RR+"Este es un error de variable! comunicaselo al administrador de VTA para que lo solucione.")
		GUI()

# Error de existencia de base de datos (openpyxl.utils.exceptions.InvalidFileException)
	except openpyxl.utils.exceptions.InvalidFileException:
		lento(RR+"¡Ocurrió un error al validar tu archivo¡" + BB +"\nEl nombre de tu Base de Datos asignada es el siguiente: " +WW+ arch02)
		if arch02 == "":
			sutil(YY+"Esto significa que no existe algun nombre especificado...")
			medio(BB+"¿Quieres solucionar este problema?")
			print (WW+"1) Si")
			print ("2) No\n")
			dcs01 = int(input(BB +"Escoge una opcion: "+ WW))
			if dcs01 == 1:
				print (GG +"¡Hay dos maneras de hacerlo!")
				print (WW +"1) Resolucion inteligente")
				print ("2) Manualmente")
				dcs02 = int(input(BB +"\nEscoge una opcion: "+ WW))
				if dcs02 == 1:
					sutil(YY +"Solucionando errores...")
					arcfn = []
					for file in os.listdir():
						if fnmatch.fnmatch(file, "*.xlsx" or "*.xls" or "*xlsm"):
							arcfn.append(file)
							prl = open("db.txt","r")
							prl1 = prl.read()
							prl2 = open("db.txt","w")
							prl3 = prl2.write(file)
							prl2.close()
							medio(GG +"El problema fue solucionado!")
							input("Presiona cualquier botón para regresar al Menú principal")
							os.system("python3 Gestor\ de\ BD.py")
				
				elif dcs02 == 2:
					prl413 = input("Inserta el nombre y extension de la base de datos: ")
					prl412 = open("db.txt","r")
					prl414 = prl412.read()
					prl415 = open("db.txt","w")
					prl416 = prl415.write(prl413)
					prl415.close()
					medio(GG +"El problema fue solucionado!")
					input("Presiona cualquier botón para regresar al Menú principal")
					os.system("python3 Gestor\ de\ BD.py") 
			...
			sutil(RR +"Puede ser que el programa no haya iniciado correctamente\nvuelve a ejecutar este prorama, si vuelve a pasar este error\ncomentaselo al administrador de Valletta")

	except PermissionError:
		print ("trata de no tener el archivo excel abierto cuando edites los datos")
# Error de identificacion de base de datos (FileNotFoundError)
	except FileNotFoundError:

		lento(RR+"¡Ocurrió un error al validar tu archivo¡" + BB +"\nEl nombre de tu Base de Datos asignada es el siguiente: " +WW+ arch02)
		sutil(YY+"Esto significa que no existe la base de datos con el nombre especificado...")
		medio(BB+"¿Quieres solucionar este problema?")
		print (WW+"1) Si")
		print ("2) No\n")
		dcs01 = int(input(BB +"Escoge una opcion: "+ WW))
